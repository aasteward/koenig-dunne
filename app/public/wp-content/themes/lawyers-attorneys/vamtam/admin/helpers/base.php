...
function wpv_validate_install() {
	...
	if ( strpos( str_replace( WP_CONTENT_DIR.'/themes/', '', get_template_directory() ), '/' ) !== false ) {
		$wpv_errors[] = __( 'The theme must be installed in a directory which is a direct child of wp-content/themes/', 'lawyers-attorneys' );
	}

	if ( ! is_writable( WPV_CACHE_DIR ) ) {
		$wpv_errors[] = sprintf( __( 'You must set write permissions (755 or 777) for the cache directory (%s)', 'lawyers-attorneys' ), WPV_CACHE_DIR );
	}

	if ( ! extension_loaded( 'gd' ) || ! function_exists( 'gd_info' ) ) {
		$wpv_errors[] = __( "It seems that your server doesn't have the GD graphic library installed. Please contact your hosting provider, they should be able to assist you with this issue", 'lawyers-attorneys' );
	}
			...
			function wpv_invalid_install() {
				...
				?>
					<div class="updated fade error" style="background: #FEF2F2; border: 1px solid #DFB8BB; color: #666;"><p>
						<?php _e( 'There were some some errors with your Vamtam theme setup:', 'lawyers-attorneys' )?>
						<ul>
		...
		<div class="row-desc">
			...
			<div>
				<section class="content"><?php echo $desc // xss ok ?></section>
				    ...
				    <footer><a href="<?php echo esc_attr( 'http://support.vamtam.com' ) ?>" title="<?php esc_attr_e( 'Read more on our Help Desk', 'lawyers-attorneys' ) ?>" target="_blank"><?php _e( 'Read more on our Help Desk', 'lawyers-attorneys' ) ?></a>
				    </footer>
			</div>
		</div>
	<?php endif;
