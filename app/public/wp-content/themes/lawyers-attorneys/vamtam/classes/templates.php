...
class WpvTemplates {
	...
	public static function pagination_list() {
		...
		if ( $total_pages > 1 ) {
			...
			echo '<span class="pages">'.sprintf( __( 'Page %d of %d', 'lawyers-attorneys' ), $current_page, $total_pages ).'</span>';
			...
			echo paginate_links(
				array(
					...
					'total' => $total_pages,
					'prev_text' => __( 'Prev', 'lawyers-attorneys' ),
					'next_text' => __( 'Next', 'lawyers-attorneys' ),
				)
			);
			// xss ok
	...
	public static function pagination( $pagination_type = null ) {
		...
		if ( $pagination_type == 'paged' ) {
			...
			if ( (int)$max > (int)$paged ) {
				...
				echo '<div class="load-more"><a href="'.esc_attr( $url ).'" class="'.$class.'"><span>'.__( 'Load more', 'lawyers-attorneys' ).'</span></a></div>';
			}
		}
	}
	...
	public static function custom_link_pages( $args = '' ) {
		$defaults = array(
			'before' => '<p id="post-pagination">' . __( 'Pages:', 'lawyers-attorneys' ),
			'after' => '</p>',
			'text_before' => '',
			'text_after' => '',
			'next_or_number' => 'number',
			'nextpagelink' => __( 'Next page', 'lawyers-attorneys' ),
			'previouspagelink' => __( 'Previous page', 'lawyers-attorneys' ),
			'pagelink' => '%',
			'echo' => 1,
		);
