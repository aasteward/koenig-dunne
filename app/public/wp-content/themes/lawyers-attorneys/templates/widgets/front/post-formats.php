<ul class="clearfix">
	<li class="post-format-pad"><a title="<?php _e('Standard', 'lawyers-attorneys')?>" href="<?php echo esc_url( add_query_arg( 'format_filter', 'standard', home_url() ) ) ?>" class="standard"><?php echo do_shortcode('[icon name="'.WpvPostFormats::get_post_format_icon('standard').'"]')?></a></li>
	<?php
		$tooltip = empty( $instance['tooltip'] ) ? __('View all %format posts', 'lawyers-attorneys') : esc_attr($instance['tooltip']);
