...
<p>
	<label for="<?php echo $this->get_field_id( 'custom_count' ); ?>"><?php _e( 'How many links to show?', 'lawyers-attorneys' ); ?></label>
...
<div class="hidden_wrap">
	<?php for ( $i=1; $i<=$this->max_custom; $i++ ):
		...
		<div class="hidden_el" <?php if ( $i > $custom_count ):?>style="display:none"<?php endif; ?>>
			...
			<p>
				<label for="<?php echo $this->get_field_id( $custom_name ); ?>"><?php printf( __( 'Name:', 'lawyers-attorneys' ) , $i ); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( $custom_url ); ?>"><?php printf( __( 'URL:', 'lawyers-attorneys' ) , $i ); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( $custom_icon ); ?>"><?php printf( __( 'Icon:', 'lawyers-attorneys' ) , $i ); ?></label>
				<a href="#" class="wpv-icon-selector-trigger vamtam-icon <?php echo esc_attr( wpv_get_icon_type( $custom_icons[$i] ) ) ?> <?php if ( empty( $custom_icons[$i] ) ) echo 'no-icon' // xss ok ?>" data-icon-name="<?php echo esc_attr( $custom_icons[$i] ) ?>" title="<?php esc_attr_e( 'Change Icon', 'lawyers-attorneys' ) ?>"><?php wpv_icon( $custom_icons[$i] ) ?></a>
