...
?><div class="post-content-outer single-post">
	...
	<?php if ( $meta_author || $meta_date || ( $meta_comments && comments_open() ) ) : ?>
		<div class="meta-top clearfix">
			...
			<?php endif ?>
			<?php if ( $meta_date ) : ?>
				<span class="post-date date updated" itemprop="datePublished"><?php the_time( get_option( 'date_format' ) ); ?> </span>
			<?php endif ?>
			<?php if ( $meta_comments && comments_open() ): ?>
				<div class="comment-count">
					<a href="<?php comments_link() ?>" class="icon theme"><?php wpv_icon( 'comments' )?></a><?php comments_popup_link( __( '0 <span class="comment-word visuallyhidden">Comments</span>', 'lawyers-attorneys' ), __( '1 <span class="comment-word visuallyhidden">Comment</span>', 'lawyers-attorneys' ), __( '% <span class="comment-word visuallyhidden">Comments</span>', 'lawyers-attorneys' ) ); ?>
				</div>
			<?php endif; ?>
		</div>
	<?php endif ?>
	...
	<?php if ( $meta_tax ) : ?>
		<div class="meta-bottom clearfix">
			...
			<?php the_tags('<div class="the-tags"><span class="icon">' . wpv_get_icon('tag') . '</span><span class="visuallyhidden">' . __( 'Tags', 'lawyers-attorneys' ) . '</span>', ', ', '</div>' ) ?>
		</div>
	<?php endif ?>
