...
class WPV_Sitemap {
	...
	private function categories($atts){
		...
		$feed = ($show_feed === true || $show_feed == 'true') ? __( 'RSS', 'lawyers-attorneys' ) : '';
	...
	private function display($query, $show_comment = false) {
		...
		while ($archive_query->have_posts()) {
			...
			if(!empty($title)) {
				$output .=
					"<li>
						<a href='".get_permalink()."' rel='bookmark' title='".sprintf( __('Permanent Link to %s', 'lawyers-attorneys'), esc_attr($title) )."'>".
							strip_tags($title).
						"</a>".
