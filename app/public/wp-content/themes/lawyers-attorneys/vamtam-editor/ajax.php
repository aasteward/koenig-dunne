	...
	private function markup_handler( $id, $attributes=null, $content='' ) {
		...
		?>
			<div id="<?php echo esc_attr( $this->get_uniqid( $id ) ) ?>" class="wpv_ed_column wpv_sortable inner-sortable column column-<?php echo esc_attr( preg_replace( '/column-(\d)(\d)/', '$1-$2', $id ) ) ?> expandable expanded" data-basename="column">
				<?php echo $this->getControls( 'size name add edit clone delete handle', preg_replace( '/column-(\d)(\d)/', '$1/$2', $id ), __( 'Column', 'lawyers-attorneys' ), $attributes ) // xss ok ?>
	...
	private function getControls( $controls, $size_str, $name='', $atts = array() ) {
		...
		$controls_html = array(
			...
			'size' => "<div class='column-size-wrapper'>
							<div class='column-increase-decrease'><a class='column-increase icon-plus' href='#' title='" . esc_attr__( 'Increase width', 'lawyers-attorneys' ) . "'></a> <a class='column-decrease icon-minus' href='#' title='" . esc_attr__( 'Decrease width', 'lawyers-attorneys' ) . "'></a></div>
							<span class='column-size'>$size_str</span>
						</div>",
			// 'add' => " <a class='column-add icon-plus' href='#' title='".__( 'Insert element', 'lawyers-attorneys' )."'></a> ",
			'clone' => " <a class='column-clone icon-copy' href='#' title='" . esc_attr__( 'Clone', 'lawyers-attorneys' )."'></a> ",
			'edit' => ' <a class="column-edit icon-edit" href="#" title="' . esc_attr__( 'Edit shortcode properties', 'lawyers-attorneys' ).'"></a>',
			'delete' => ' <a class="column-remove icon-remove" href="#" title="' . esc_attr__( 'Remove shortcode', 'lawyers-attorneys' ).'"></a>',
			'handle' => '<div class="handlediv" title="' . esc_attr__( 'Click to toggle', 'lawyers-attorneys' ).'"><br /></div>',
		);
				...
				foreach ( $sub_contents as $i=>$text ) : ?>
					...
					<div>
						<h3 class="title-wrapper clearfix">
							...
							<a class="accordion-remove icon-remove" title="<?php esc_attr_e( 'Remove', 'lawyers-attorneys' ) ?>"></a>
							<a class="accordion-clone icon-copy" title="<?php esc_attr_e( 'Clone', 'lawyers-attorneys' ) ?>"></a>
							<a class="accordion-background-selector" data-background-image="<?php if ( isset( $params[$i]['background_image'] ) ) echo esc_attr( $params[$i]['background_image'] ) ?>" title="<?php esc_attr_e( 'Change Pane Background', 'lawyers-attorneys' ) ?>"><?php wpv_icon( 'image' ) ?></a>
						</h3>
		 ...
		 'icon' => '',
				...
				?>
					<li>
						...
						<a class="tab-remove icon-remove" title="<?php esc_attr_e( 'Remove', 'lawyers-attorneys' ) ?>"></a>
						<a class="tab-clone icon-copy" title="<?php esc_attr_e( 'Clone', 'lawyers-attorneys' ) ?>"></a>
						<a class="wpv-icon-selector-trigger tab-icon-selector vamtam-icon <?php echo esc_attr( wpv_get_icon_type( $p['icon'] ) ) ?> <?php if ( empty( $p['icon'] ) ) echo 'no-icon' ?>" data-icon-name="<?php echo esc_attr( $p['icon'] ) ?>" title="<?php esc_attr_e( 'Change Icon', 'lawyers-attorneys' ) ?>"><?php wpv_icon( $p['icon'] ) ?></a>
					</li>
				<?php endforeach;
