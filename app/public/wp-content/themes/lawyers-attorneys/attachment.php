...
$wpv_title = __('Attachment', 'lawyers-attorneys');
	...
	<div class="row page-wrapper">
		...
		<article id="post-<?php the_ID(); ?>" <?php post_class(WpvTemplates::get_layout()); ?>>
			...
			<div class="page-content">
					...
					<?php
						if ( wp_attachment_is_image() ) {
							...
							if ( isset( $metadata['width'] ) && isset( $metadata['height'] ) ) {
								printf( __( 'Original size is %s pixels', 'lawyers-attorneys'),
									sprintf( '<a href="%1$s" title="%2$s">%3$s &times; %4$s</a>',
										wp_get_attachment_url(),
										esc_attr( __('Link to full-size image', 'lawyers-attorneys') ),
										$metadata['width'],
										$metadata['height']
									)
								);
				...
				<?php the_content( __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'lawyers-attorneys' ) ); ?>
